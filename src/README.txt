...
A: A few things:
- Use the Adapter pattern on MiningTask and implement different algorithms for testing the most efficient polyndrome algorithm, or even bring a completely different challenge
- Add more configuration parameters to the MiningExecutor's constructor for fine tuning the ThreadPoolExecutor such as (e.g. poolSize, keepAliveTime)
- MiningStatistics should check if a task was interrupted and exclude it from the statistics
- Upon completing the computation, the string to print out is returned. Instead, its own data type should be returned with print methods.
- Change the data type of the main arguments to support larger values
- Implement a more robust handling of main arguments
 

2) How would you prove that your solution is correct?
A: There are two cases to check correctness: 1) that all output numbers are correct, and 2) that we are not missing a number in the output.
For case 1) I could write another program that inspects the output to make sure it fulfills the rule that both numbers (decimal-based and binary-based) are polindromes
For case 2) and if performance is not a concern, I would implement a different algorithm and compare both results 
